  ...
  public class RecordsMap : CsvClassMap<Record>
  {
    ...
    */
    public RecordsMap()
    {
      ...
      Map(rec => rec.ItemNumber).Name("Item_Number");
      Map(rec => rec.Location).Name("Location");
      Map(rec => rec.OccurredDateTime).Name("Occurred_Date_Time");
      Map(rec => rec.OffenderAge).Name("Offender_Age").TypeConverter<NullableStringToInt32Converter>();
      Map(rec => rec.OffenderGender).Name("Offender_Gender");
      Map(rec => rec.OffenderRace).Name("Offender_Race");
      Map(rec => rec.SignalDescription).Name("Signal_Description");
      Map(rec => rec.SignalType).Name("Signal_Type");
      Map(rec => rec.VictimAge).Name("Victim_Age").TypeConverter<NullableStringToInt32Converter>();
      Map(rec => rec.VictimGender).Name("Victim_Gender");
